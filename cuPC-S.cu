...
extern "C" _declspec(dllexport) void Skeleton(double* C, int *P, int *G, double *Th, int *l, int *maxlevel, double *pMax, int* SepSet)
{
    ...
    printf("in cuda Skeleton, n=%d,maxLevel=%d\n",n,*maxlevel);
    system_clock::time_point st1 = system_clock::now();
    // n*n*(4+14*4+4+8+4+8)=n*n*84
    *l = 0;
    ...
    CudaCheckError();
    system_clock::time_point et1 = system_clock::now();
    long long cost_milliseconds1 = duration_cast<milliseconds>(et1 - st1).count();
    printf("init cost=%lldms\n", cost_milliseconds1);
    //----------------------------------------------------------
    for (*l = 0; *l <= ML && !FinishFlag && *l <= *maxlevel; *l = *l + 1){
        ...
        if (*l == 0){
            ...
            //cudaMemset(SepSet_cuda, 0xff, n * n * ML * sizeof(int));
            CudaCheckError();
        } else {
            //================================> Start Scan Process <===============================
            ...
            if (*l == 1){
                ...
                // HANDLE_ERROR( cudaMalloc((void**)&SepSet_cuda,  n * n * 1 * sizeof(int)) );
                // 将n作为一个常量内存，不需要每次都从全局内存中读取，使用cudaMemcpyToSymbol函数
                cudaMemcpyToSymbol(n1,&n,sizeof(int), 0, cudaMemcpyHostToDevice);
                // cal_Indepl2是最为耗时的函数，所以大部分修改都是围绕这个函数
                cal_Indepl2 <<< BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int) >>> (C_cuda,  G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, (float)Th[2]*2);
                // HANDLE_ERROR( cudaFree(SepSet_cuda) );
                CudaCheckError();
            }
            else if(*l == 3){
        ...
        long long cost_milliseconds = duration_cast<milliseconds>(et - st).count();
        printf("l=%d, maxlevel=%d,ML=%d,FinishFlag=%d,cost=%lldms\n", *l, ML, *maxlevel, FinishFlag, cost_milliseconds);
    }// if l > 0

    system_clock::time_point pmax_st = system_clock::now();
    ...
    system_clock::time_point pmax_et = system_clock::now();
    printf("download use time=%ldms\n", duration_cast<milliseconds>(pmax_et - pmax_st).count());
...
__global__ void cal_Indepl1(double *C, int *G, int *GPrime, int *mutex, int* Sepset, double* pMax, double th, int n)
{
    ...
    for(int d1 = 0; d1 < NumOfGivenJump; d1++){
        ...
        if( NbrIdxPointer < SizeOfArr){
            ...
            for(int d2 = 0; d2 < SizeOfArr; d2+=4){
                if( d2 != NbrIdxPointer && d2<SizeOfArr){
                    YIdx = G_Chunk[d2];
                    int iindx = XIdx * n + YIdx;
                    if (G[iindx] == 1) {    
                        NoEdgeFlag = 0;
                        M0 = C[iindx];         
                        ...
                        rho     = H[0][1] / (sqrtf(fabs(H[0][0])) * sqrtf(fabs(H[1][1])));
                        Z       = fabs( 0.5 * (logf( fabs((1 + rho))) - logf(fabs(1 - rho)) ) );

                        if (Z < th){
                            if(atomicCAS(&mutex[iindx], 0, 1) == 0){
                                G[iindx] = 0;
                                G[YIdx * n + XIdx] = 0;
                                pMax[iindx] = Z;
                                Sepset[(iindx) * ML] = NbrIdx;
                            }
                        }
                    }
                }
                int d3 = d2+1;
                if( d3 != NbrIdxPointer && d3<SizeOfArr){
                    YIdx = G_Chunk[d3];
                    int iindx = XIdx * n + YIdx;
                    if (G[iindx] == 1) {    
                        NoEdgeFlag = 0;
...
__global__ void cal_Indepl2(double *C, int *G, int* GPrime, int *mutex, int* Sepset, double* pMax, float th)
{
    int YIdx;
    int XIdx = by;
    int NbrIdxPointer[2];
    ...
    int NumOfComb;
    __shared__ int NoEdgeFlag;
    // 将H[2][2]，rho和Z改为float，简化计算
    // 其他的几个改为float后，函数无法结束，可能是太接近0，导致改为float后直接变成0
    double M0;
    double M1[2][2];
    double M2[2][2];
    double M2Inv[2][2];
    ...
    // 这个G_Chunk的目的是把一部分全局内存复制到共享内存中，共享内存的访存速度要快于全局内存，这样做能减少延迟
    // 这是作者本来就写好的，是一种常用的优化手段
    extern __shared__ int G_Chunk[];
    NoEdgeFlag = 0;
    SizeOfArr = GPrime[XIdx * n1 + n1 - 1];
    if (SizeOfArr <= 2){
        return;
    }
    ...
    for (int cnt = 0; cnt < NumberOfJump; cnt++){
        if( ( tx + cnt * ParGivenL2 ) < SizeOfArr){
            G_Chunk[ tx + cnt * ParGivenL2 ] =  GPrime[ XIdx * n1 + tx + cnt * ParGivenL2];
        }
        __syncthreads();
    }
    ...
    // 本来M2的这两个值，在每一次循环中都被赋值成为1，没有必要，循环前赋值一次就行
    M2[1][1] = 1;
    M2[0][0] = 1;
    // int calc_count = 0;
    for(int d1 = 0; d1 < NumOfGivenJump; d1++){
        ...
        if( (tx + bx * ParGivenL2 + d1 * ParGivenL2 * NumOfBlockForEachNodeL2) < NumOfComb){
            ...
            NoEdgeFlag = 1;
            // 修改原本的IthCombination的一些逻辑，减少线程分支
            // IthCombination(NbrIdxPointer, SizeOfArr, 2, tx + bx * ParGivenL2 + d1 * ParGivenL2 * NumOfBlockForEachNodeL2 + 1);
            IthCombination2(NbrIdxPointer, SizeOfArr, tx + bx * ParGivenL2 + d1 * ParGivenL2 * NumOfBlockForEachNodeL2 + 1);

            NbrIdx[0] = G_Chunk[NbrIdxPointer[0]];
            NbrIdx[1] = G_Chunk[NbrIdxPointer[1]];
            M2[0][1] = C[ NbrIdx[0] * n1 + NbrIdx[1] ];
            M2[1][0] = M2[0][1];

            M1[0][1] = C[ XIdx * n1 + NbrIdx[1] ];
            M1[0][0] = C[ XIdx * n1 + NbrIdx[0] ];
            // pseudoinversel2也做了一些优化，减少线程分化
            pseudoinversel2(M2, M2Inv);

            // 把这个循环的代码复制4次，d2的步进变为4，也可以减少一些线程分化
            for(int d2 = 0; d2 < SizeOfArr; d2+=4){
                if( d2<SizeOfArr && (d2 != (NbrIdxPointer[0])) && (d2 != (NbrIdxPointer[1]))  ){
                    YIdx = G_Chunk[d2];
                    int iindx = XIdx * n1 + YIdx; // XIdx * n1 + YIdx这个计算出来的值用了很多次，把它赋值，防止多次计算
                    if (G[iindx] == 1) {    
                        NoEdgeFlag = 0;
                        M0 = C[iindx];         
                        M1[1][0] = C[ YIdx * n1 + NbrIdx[0] ];
                        M1[1][1] = C[ YIdx * n1 + NbrIdx[1] ];
                        ...
                        // 这里的计算，由原本的sqrt和log改为sqrtf和logf，后两个函数是计算float的函数，会比前两个函数快很多
                        rho   =  H[0][1] / ( sqrtf( abs(H[0][0] * H[1][1]) ) );
                        Z     =  abs( logf( abs( (1 + rho)  /  (1 - rho) ) ) );
                        if (Z < th){
                            if(atomicCAS(&mutex[iindx], 0, 1) == 0){//lock
                                G[iindx] = 0;
                                G[YIdx * n1 + XIdx] = 0;
                                pMax[iindx] = 0.5 * Z;
                                Sepset[(iindx) * ML] = NbrIdx[0];
                                Sepset[(iindx) * ML + 1] = NbrIdx[1];
                            }
                        } 
                    }
                }  
                int d3 = d2+1;
                if( d3<SizeOfArr && d3 != NbrIdxPointer[0] && d3 != NbrIdxPointer[1] ){
                    YIdx = G_Chunk[d3];
                    int iindx = XIdx * n1 + YIdx;
                    if (G[iindx] == 1) {    
                        NoEdgeFlag = 0;
                        M0 = C[iindx];         
                        M1[1][0] = C[ YIdx * n1 + NbrIdx[0] ];
    ...
    // printf("NumOfGivenJump=%d,SizeOfArr=%d,calc_count=%d\n",NumOfGivenJump,SizeOfArr,calc_count);
}

__global__ void depl2_kernel(int SizeOfArr, int NbrIdxPointer_0, int NbrIdxPointer_1, int NbrIdx_0, int NbrIdx_1,
    int XIdx, double M1_0_0,double M1_0_1, double M2Inv_0_0,double M2Inv_0_1,double M2Inv_1_0,double M2Inv_1_1,
    double *C, int *G, int* GPrime, int *mutex, int* Sepset, double* pMax, double th, bool* NoEdgeFlag, int flag_index)
{
    int d2 = blockDim.x * blockIdx.x + threadIdx.x;
    if (d2>=SizeOfArr||(d2 == (NbrIdxPointer_0 - 1)) || (d2 == (NbrIdxPointer_1 - 1))) {
        return;
    }
    int YIdx = GPrime[XIdx * n1 + d2];
    double M1MulM2Inv[2][2];
    double H[2][2];
    if (G[XIdx * n1 + YIdx] == 1) {
        NoEdgeFlag[flag_index] = false;
        __syncthreads();
        double M0 = C[XIdx * n1 + YIdx];
        double M1_1_0 = C[YIdx * n1 + NbrIdx_0];
        double M1_1_1 = C[YIdx * n1 + NbrIdx_1];
        for (int c1 = 0; c1 < 2; c1++)
        {
            for (int c2 = 0; c2 < 2; c2++)
            {
                ...
                for (int c3 = 0; c3 < 2; c3++){
                    ...
                    M1MulM2Inv[c1][c2] += mmm * mmm_inv;
                }
                
            }
        }
        H[0][0] = 1 - (M1MulM2Inv[0][0] * M1_0_0+ M1MulM2Inv[0][1] * M1_0_1);
        H[0][1] = M0 - (M1MulM2Inv[0][0] * M1_1_0 + M1MulM2Inv[0][1] * M1_1_1);
        H[1][1] = 1 - (M1MulM2Inv[1][0] * M1_1_0 + M1MulM2Inv[1][1] * M1_1_1);

        double rho = H[0][1] / (sqrt(abs(H[0][0] * H[1][1])));
        double Z = 0.5 * abs(log(abs((1 + rho) / (1 - rho))));

        if (Z < th) {
            if (atomicCAS(&mutex[XIdx * n1 + YIdx], 0, 1) == 0) {//lock
                G[XIdx * n1 + YIdx] = 0;
                G[YIdx * n1 + XIdx] = 0;
                pMax[XIdx * n1 + YIdx] = Z;
                Sepset[(XIdx * n1 + YIdx) * ML] = NbrIdx_0;
                Sepset[(XIdx * n1 + YIdx) * ML + 1] = NbrIdx_1;
            }
        }
    }
}
...
__device__ void pseudoinversel2(double M2[][2], double M2Inv[][2])
{
    ...
    if (tol > A[0][0] && A[0][0] > 0) {
        tol = A[0][0];
    }
    if (tol > A[1][1] && A[1][1] > 0) {
        tol = A[1][1];
    }
    ...
    if (r == 0) {
        ...
        L[0][r] = A[0][0] - temp[0][0];
        L[1][r] = A[1][0] - temp[1][0];
    }
    //check with threshold
    if (L[0][r] > tol) {
        L[0][r] = sqrt(L[0][r]);
    ...
    if (r == 0) {
        ...
        // for (int i = k; i < size; ++i) {
        for (int l = 0; l < r; l++) {
            temp[1][1] += L[1][l] * L[1][l];
        }
        L[1][r] = A[1][1] - temp[1][1];
    }
    //check with threshold
    if (L[1][r] > tol) {
        L[1][r] = sqrt(L[1][r]);
...
__device__  void IthCombination2(int out[], int N, int L)
{
    //The out[p] can be calculated  using formula out[p] = out[p - 1] + L - K. note that out[p] is in 1-base indexing
    // 减少线程分化：
    // 1. P1永远=1，没必要写循环
    // 2. out两个值在外面都要-1，这里直接先计算-1后的值
    // 3. if(i>0)永远不成立，不需要判断
    // 4. BINOM函数的逻辑很简单，而且这里传进去的值有一个永远是1，所以直接把BINOM函数的内容写到循环里
    int R;
    int k = 0;
    out[0] = 0;
    while(k < L){
